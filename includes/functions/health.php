...
function multidealer_health()
{
    global $multidealer_memory_result;
    ...
    echo '<br />';
    echo '</strong>';
    $multidealer_memory_result = ob_get_contents();
    ob_end_clean();
    function multidealer_add_memory_test($tests)
    {
        $tests['direct']['memory_plugin'] = array(
            'label' => __('My Memory Test', 'antihacker'),
            'test' => 'multidealer_memory_test',
        );
        return $tests;
    }
    $perc = $memory['usage'] / $memory['wp_limit'];
    if ($perc > .7) {
        add_filter('site_status_tests', 'multidealer_add_memory_test');
    }
    
    function multidealer_memory_test()
    {
        global $multidealer_memory_result;
        $result = array(
            'badge' => array(
