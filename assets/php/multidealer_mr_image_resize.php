...
function multidealer_mr_common_info($url, $width, $height, $crop, $align, $retina)
{
    ...
    */
    ...
    if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
        ...
        $pos = strpos($url, '/assets/images/image-no-available.jpg');
        if($pos !== false)
        {   
            $pos = strrpos($url,'/');
            $image_name = substr($url, $pos+1);
            $file_path = trim(plugin_dir_path(__file__));
            $size = strlen($file_path);
            $file_path = substr($file_path,0,$size-4);
        ...
        else
        {
            ...
            multidealer_write_log('Upload directory not found (MultiDealer Plugin) - '.__FILE__ .' - Line  '.__LINE__);
